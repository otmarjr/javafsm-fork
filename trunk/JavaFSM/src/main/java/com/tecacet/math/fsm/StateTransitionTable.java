...
public class StateTransitionTable<S, C> implements StateTransitionDiagram<S, C> {
    ...
    class Pair {
        ...
        public int hashCode() {
            ...
            return (state.hashCode() + (31 ^ state.toString().length()) * symbol.hashCode());
        }
    } // Pair
    ...
    public S getNextState(S from, C symbol) throws InvalidTransitionException {
        if (!transitionMap.containsKey(new Pair(from, symbol))) {
            throw new InvalidTransitionException(String.format("There is no transition from state %s on symbol %s.",
                    from, symbol));
        }
    ...
    //                    + transitionMap.containsKey(new Pair(pair.state, pair.symbol)));
    //            sb.append("map contains this value = " + transitionMap.containsValue(pair.state));
    //        }
    //    }
